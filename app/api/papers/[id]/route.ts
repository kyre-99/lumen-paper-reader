...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const payload = await request.json() as { folderId?: unknown; title?: unknown };
  const db = getDb();
  ...
  const updates: { folderId?: string | null; title?: string; updatedAt: string } = { updatedAt: new Date().toISOString() };
